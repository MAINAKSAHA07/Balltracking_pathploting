...
def main() -> None:
    ...
    if not video_path:
        ...
        videos = [f for f in media_dir.iterdir() if f.is_file() and f.suffix in (".mp4", ".mov")]
        if not videos:
            logger.error("No videos found in media directory")
            return
        ...
        while True:
            try:
                choice = int(input(f"Select a video [0-{len(videos) - 1}]: "))
    ...
    if args.save_video:
        video_writer = cv2.VideoWriter(
            ...
            fps=fps,
            frameSize=(int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) + 300),
        )
    else:
        video_writer = None
    ...
    fig = plt.figure(figsize=(frame0.shape[1] / 100, 3), dpi=100)
    ...
    while True:
        ...
        plot = cv2.resize(plot, (frame_annotated.shape[1], plot.shape[0]))
